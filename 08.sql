...
select * from customer where address in('서울시 강남구', '서울시 송파구', '서울시 서초구');
...
-- select * from customer where id > 5 and address in ('서울시 강남구', '서울시 서초구', '서울시 송파구', '서울시 강서구');
select * from customer where id >  5 and address like '서울시%';
...
select * from customer where email is not null and id > 2 and address like '%종로구';

-- 6. 주소가 '서울'이면서 id가 5이하인 고객 조회하는 쿼리
select * from customer where address like '서울%' and id <= 5;
